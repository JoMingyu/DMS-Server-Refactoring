...
@API(functionCategory = "계정", summary = "로그인 - 관리자")
@REST(requestBody = "id : String, password : String, keep_login : boolean", successCode = 201, failureCode = 204, etc = "쿠키나 세션 존재")
@Route(uri = "/signin/admin", method = HttpMethod.POST)
public class Signin_Admin implements Handler<RoutingContext> {
	@Override
	public void handle(RoutingContext ctx) {
		...
		ResultSet rs = MySQL.executeQuery("SELECT COUNT(*) FROM admin_account WHERE id=? AND password=?", id, password);
        try {
        	if(rs.next()) {
        		...
        		MySQL.executeUpdate("UPDATE admin_account SET session_id=? WHERE id=?", SHA256.encrypt(sessionId), id);
	...
	private String createSessionId() {
		while(true) {
			...
			ResultSet rs = MySQL.executeQuery("SELECT * FROM admin_account WHERE session_id=?", uuid);
			try {
				if(!rs.next()) {
					return uuid;
				}
